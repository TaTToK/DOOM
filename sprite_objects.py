import pygame
from ИГРА.settings import *
from collections import deque
from ИГРА.Ray_casting import mapping
    ...
    def __init__(self):
        self.sprite_parameters = {
            'sprite_barrel': {
                ...
                'side': 30,
                'animation': deque(
                    [pygame.image.load(f'../ИГРА/sprites/barrel/anim/{i}.png').convert_alpha() for i in range(12)]),
                'death_animation': deque([pygame.image.load(f'../ИГРА/sprites/barrel/death/{i}.png')
            ...
            'sprite_pin': {
                ...
                'scale': (0.6, 0.6),
                'side': 30,
                'animation': deque([pygame.image.load(f'sprites/pin/anim/0.png').convert_alpha() for i in range(8)]),
                'death_animation': [],
                'is_dead': 'immortal',
                'dead_shift': None,
                'animation_dist': 800,
            ...
            'sprite_flame': {
                ...
                'side': 30,
                'animation': deque(
                    [pygame.image.load(f'sprites/flame/anim/0.png').convert_alpha() for i in range(16)]),
                'death_animation': [],
                'is_dead': 'immortal',
                'dead_shift': 1.8,
                'animation_dist': 1800,
            ...
            'npc_devil': {
                'sprite': [pygame.image.load(f'../ИГРА/sprites/devil/base/{i}.png').convert_alpha() for i in range(8)],
                'viewing_angles': True,
                'shift': 0.0,
                'scale': (1.1, 1.1),
                'side': 50,
                'animation': [],
                'death_animation': deque([pygame.image.load(f'../ИГРА/sprites/devil/death/{i}.png')
                ...
                'flag': 'npc',
                'obj_action': deque(
                    [pygame.image.load(f'../ИГРА/sprites/devil/anim/{i}.png').convert_alpha() for i in range(9)]),
            },
            'sprite_door_v': {
                'sprite': [pygame.image.load(f'sprites/doors/door_v/0.png').convert_alpha() for i in range(16)],
                'viewing_angles': True,
                'shift': 0.1,
                'scale': (2.6, 1.2),
                'side': 100,
            ...
            'sprite_door_h': {
                'sprite': [pygame.image.load(f'../ИГРА/sprites/doors/door_h/{i}.png').convert_alpha() for i in range(16)],
                'viewing_angles': True,
                'shift': 0.1,
                'scale': (2.6, 1.2),
                'side': 100,
            ...
            'npc_soldier0': {
                'sprite': [pygame.image.load(f'sprites/npc/soldier0/base/0.png').convert_alpha() for i in range(8)],
                'viewing_angles': True,
                'shift': 0.8,
                'scale': (0.4, 0.6),
                'side': 30,
                'animation': [],
                'death_animation': deque([pygame.image.load(f'sprites/npc/soldier0/death/0.png')
                ...
                'blocked': True,
                'flag': 'npc',
                'obj_action': deque([pygame.image.load(f'sprites/npc/soldier0/action/0.png')
                                    .convert_alpha() for i in range(4)])
            },
        }
